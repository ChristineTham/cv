...
_This is the paper I gave at AUUG'94, the Australian UNIX Users Group conference, written
while I was an Architecture Consultant at [AT&T Global Information
Solutions](/cv/work/ncr/). I revised it for Oracle OpenWorld '94, and that version is
[also on this site](/cv/article/openworld-1994/). The two agree closely, so this
article marks the places where the Oracle revision said something different._
...
_Figure 4 is not an embedded object and has no metafile, so it comes from slide 24 of the
conference deck instead. Three things had to be put right in it. The slide's title and its
three bullets are presentation furniture and are dropped, since the paper prints the diagram
alone. The importer does not carry text rotation, so "Security" and "Management" arrived lying
flat across the middle of the diagram and are turned back upright into their side columns. And
it re-wrapped "Naming" across two lines mid-word, which is set back on one. The GIF is the
authority for all three._

_That download also carries the deck I presented from, 42 slides with the speaker notes still
in them, given in the Day 3 session at 2pm. Its slides are in the presentation's own pink and
