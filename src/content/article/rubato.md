...
_The project's own files turned up later, on a floppy I archived in December 1992: the troff source this was set from, three earlier drafts of it, and the C and assembly for the player — the MPU-401 code that was to drive the JX-8P. **[Download the Rubato project archive (ZIP, 166 KB)](/cv/rubato-project.zip)**. The compiler is not in it. The design below anticipates a compiler and a player, and only the player survives._
