...
- `src/utils/tags.ts`: The canonical vocabulary (`TAG_GROUPS`), slugs, counting and the cross-collection index. Tags are shared by five collections, which is why they are not in `articles.ts` — that file is categories only. **The list is closed and `src/content.config.ts` enforces it**, so an unlisted tag fails the build naming the file. `.claude/skills/tags` covers adding, merging and renaming, and its `scripts/review.py` reports orphans, near-misses, singletons and islands.
- `src/components/wordcloud.astro`: Animated tag cloud on `/work` and the article index pages. Give a tag an `href` and its label becomes clickable and the text equivalent becomes visible; leave it off and the cloud stays an inert decoration. Both clouds pass hrefs now.
- `src/components/taglist.astro`: The row of tag pills, shared by both clouds' visible index, the `/tags/` page, the `Tagged` footer on every article, role and qualification, and the project and creation cards. One component because all of them must look identical.
- `src/components/articleindex.astro` / `entryindex.astro`: The dated lists — articles in the first, everything else in the second via a normalised `Entry`. Shared by the category and tag pages. `headingLevel` matters: a tag page nests them under a section heading, so their titles have to drop to `h3`, and hard-coding `h2` gave it two `h2`s one inside the other's section. Neither is `articlelist.astro`, the home page's recent-writing block, which fetches the collection itself.
- `src/layouts/tags.astro`: `/tags/`, the whole vocabulary grouped as declared. Reached from the nav and from every tag page's eyebrow.
...
- **`tags` is one closed vocabulary across five collections** — `article`, `work`, `project`, `education` and `creation`. 152 canonical tags, declared and grouped in `src/utils/tags.ts`, validated by a Zod enum in `src/content.config.ts`. Every distinct tag publishes `/tag/<slug>/`, listing everything carrying it from all five. So `Options pricing` reaches the articles, the two roles and the two qualifications; `Service oriented architecture` reaches five talks and the two roles they were given from.
- **The list is closed because the failure is otherwise silent.** A reworded synonym does not degrade — it publishes a second page, and both then claim to be the whole of a subject cut in half. "IT strategy" and "Technology strategy" had drifted apart across two roles at the same bank, and "Data analytics" and "Data science" across two engagements, with no error either time. Adding a tag is now a deliberate two-part edit, and doing half of it fails the build — which is the point. Use `.claude/skills/tags`.
- Slugs must be unique across the **whole** vocabulary, not within a group — `tagIndex` fails the build naming both offenders rather than letting two pages compete for one URL. Categories are the other axis, article-only and closed, in `src/utils/articles.ts`; an article normally carries one of each kind of category plus its tags. `passion` is deliberately untagged.
